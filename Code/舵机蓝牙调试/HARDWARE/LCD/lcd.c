...
/*显示浮点数，包括正负*/
void LCD_ShowFloatNum(u16 x, u16 y, float *num)
{
	char str[10];
	u8 len;
	u8 i;
	/* 浮点数格式修改此处 */
	len = sprintf((char *)str, "%3.4f",*num);
	for(i = 0; i < len; i++)
	{
		 LCD_ShowChar(x+i*8,y,str[i],16,0);
	}
}


/*显示整形数，包括正负*/
void LCD_ShowIntNum(u16 x, u16 y, int *num)
{
	char str[10];
	u8 len, i;
	/* 整数格式修改此处 */
	len = len = sprintf((char *)str, "%d",*num);
	for(i = 0; i < len; i++)
	{
		 LCD_ShowChar(x+i*8,y,str[i],16,0);
	}
